...
class WinnerPickedServlet extends HttpServlet {
  override def doPost(req: HttpServletRequest, resp: HttpServletResponse): Unit = {
    ...
    if (!winnerCat.equals("None") && !winnerSelectedString.equals("None")) {
      ...
      val winnerObject = WinnerData(findCategoryByName(winnerCat).id, findNomineeByName(winnerSelectedString).id)
      val nomsForCat = findCategoryByName(winnerCat).nominees
